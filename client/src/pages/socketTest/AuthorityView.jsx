...
const AuthorityView = () => {
    ...
    const handleRegister = (e) => {
        e.preventDefault();
        if (authorityId.trim()) {
            registerAsAuthority(authorityId);
    ...
    const handleAcceptCall = (mode) => {
        acceptCall(currentCallId, mode);
    };

    if (!connected) {
        return (
            <div className="h-full flex items-center justify-center p-4">
                <Title title="Authority View - Connecting" />
    ...
    if (!isRegistered) {
        return (
            <div className="h-full flex items-center justify-center p-4">
                <Title title="Authority Registration" />
                <div className="max-w-md w-full bg-white rounded-lg shadow-md overflow-hidden p-6">
                    ...
                    <form onSubmit={handleRegister}>
                        <div className="mb-4">
                            <label className="block text-sm font-medium text-gray-700 mb-1">Authority ID</label>
                            <input
                                type="text"
                                value={authorityId}
                        ...
                        <div className="flex items-center justify-between">
                            <Link to="/socket-test" className="text-sm text-indigo-600 hover:text-indigo-800">
                                Back to Selection
                            </Link>
                            <button
                                type="submit"
    ...
    return (
        <div className="h-full flex flex-col p-4">
            <Title title="Authority Chat Dashboard" />
            <div className="mb-4 flex justify-between items-center">
                <div>
                    <h1 className="text-xl font-semibold">Authority Dashboard</h1>
            ...
            <div className="mb-4">
                {callStatus === 'idle' && (
                    <div className="p-4 bg-gray-50 rounded-md">
                        <div className="flex items-center mb-2">
                            <div className="mr-3 h-3 w-3 bg-gray-300 rounded-full"></div>
                            <p>Waiting for incoming calls...</p>
                        </div>
                        <p className="text-sm text-gray-500">
                            Anonymous reporters can call you. When someone calls, you'll see options to accept in text or voice mode.
                        </p>
                    </div>
                )}
                ...
                {callStatus === 'incoming' && (
                    <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-md">
                        <div className="flex items-center mb-3">
                            ...
                            <p className="font-medium">Incoming anonymous {communicationMode} call</p>
                        </div>
                        <div className="flex space-x-3">
                            <button
                                onClick={() => handleAcceptCall('text')}
                                className="flex items-center justify-center px-3 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 flex-1"
                            >
                            ...
                            <button
                                ...
                                className="flex items-center justify-center px-3 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 flex-1"
                            >
                                <FaPhone className="h-4 w-4 mr-2" />
                                Accept as Voice
                            </button>
                            <button
                                onClick={() => rejectCall(currentCallId)}
                ...
                {callStatus === 'active' && (
                    <div className="flex items-center mb-4">
                        ...
                        <p>Connected to Anonymous Reporter ({communicationMode} mode)</p>
